...
export class AdminuploadService {
  ...
  pushUpload(upload: Upload,selectedoption:string,songname:string,songimg:string,songart:string) {
    ...
    let uploadTask = storageRef.child(`${this.basePath}/${selectedoption}/${upload.file.name}`).put(upload.file);
    ...
    uploadTask.on(firebase.storage.TaskEvent.STATE_CHANGED,
      ...
      .then( downloadUrl => {
        ...
        upload.name = songname
        alert("file name: "+upload.name)
        upload.albumart=songimg
        upload.artist=songart;
        upload.album=selectedoption;
        this.saveFileData(upload,selectedoption)
      })
      .catch( error => {
        console.log(error);
        //catch error here
  ...
  private saveFileData(upload: Upload,selectedoption:string) {
    alert("file url from save file: "+upload.song)
    alert("userid from save file: "+this.userId)
    this.db.list(`${selectedoption}`).push(upload);
  }
